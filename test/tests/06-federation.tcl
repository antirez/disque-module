...
test "Add multiple jobs to the same queue, different nodes" {
    for {set j 0} {$j < $numjobs} {incr j} {
        ...
        set id [R $i addjob myqueue job_$j 5000 replicate 3]
        set job [R $i show $id]
        assert {$id ne {}}
...
test "Add multiple jobs to node 1 with RETRY 0" {
    for {set j 0} {$j < $numjobs} {incr j} {
        set i 1
        set id [R $i addjob myqueue job_$j 5000 replicate 1 retry 0]
        set job [R $i show $id]
        assert {$id ne {}}
    }
}
