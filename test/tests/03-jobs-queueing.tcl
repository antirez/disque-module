...
test "ADDJOB will initially queue the job in the target node" {
    set id [R 0 addjob myqueue myjob 5000 replicate 3]
    set job [R 0 show $id]
    assert {$id ne {}}
...
test "If the job is not consumed, without partitions, no requeue happens" {
    set id [R 0 addjob myqueue myjob 5000 replicate 3 retry 1]
    set job [R 0 show $id]
    assert {$id ne {}}
...
test "If the job is consumed, but not acknowledged, it gets requeued" {
    ...
    set id [R 0 addjob $qname myjob 5000 replicate 3 retry 1]
    set job [R 0 show $id]
    assert {$id ne {}}
...
test "If retry is set to 0, no requeue happens after a job is consumed" {
    set qname [randomQueue]
    set id [R 0 addjob $qname myjob 5000 replicate 1 retry 0]
    set job [R 0 show $id]
    assert {$id ne {}}
    set myjob [lindex [R 0 getjob from $qname] 0]
    assert {[lindex $myjob 0] eq $qname}
...
test "Best effort first and/or major-node-enqueues rule works" {
    ...
    set qname [randomQueue]
    set id [R 0 addjob $qname myjob 5000 replicate 3 retry 5]
    set job [R 0 show $id]
    assert {$id ne {}}
...
for {set j 0} {$j < 10} {incr j} {
    ...
    test "Job replicated N times is delivered crashing N-1 nodes #$j" {
        ...
        foreach node_id $killed {
            restart_instance redis $node_id
        }
    }
}
...
test "Single node jobs are correctly ordered in a FIFO fashion" {
    ...
    for {set j 0} {$j < 100} {incr j} {
        R 0 addjob $qname $j 5000 replicate 1 retry 0
    }
    for {set j 0} {$j < 100} {incr j} {
        set job [R 0 getjob from $qname TIMEOUT 5000]
...
test "ADDJOB fails if queue length exceeds MAXLEN" {
    set qname [randomQueue]
    R 0 addjob $qname myjob 5000 maxlen 1
    catch {R 0 addjob $qname myjob 5000 maxlen 1} job_id
