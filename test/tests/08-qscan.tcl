...
test "QSCAN with BUSYLOOP option" {
    # Create 512 queues, each has a job enqueued
    for {set i 0} {$i < 512} {incr i} {
        R 0 addjob [randomQueue] myjob 5000 replicate 1 retry 0
    }
    set res [R 0 qscan busyloop]
...
test "QSCAN with MINLEN or MAXLEN option" {
    ...
    for {set i 0} {$i < 16} {incr i} {
        R 0 addjob [lindex $queues $i] myjob 5000 replicate 1 retry 0
    }
    set res [R 0 qscan busyloop minlen 2]
    set queues [lindex $res 1]
    ...
    set res [R 0 qscan busyloop minlen 3]
    set queues [lindex $res 1]
    assert_equal 8 [llength $queues]

    set res [R 0 qscan busyloop minlen 2 maxlen 3]
    set queues [lindex $res 1]
    assert_equal 16 [llength $queues]
    # Remove all the jobs, queues
    R 0 debug flushall
...
test "QSCAN with IMPORTRATE option" {
    ...
    for {set i 0} {$i < 4} {incr i} {
        R 1 addjob queue_0001 myjob 0
        R 1 addjob queue_0002 myjob 0
        R 1 addjob queue_0003 myjob 0
        R 1 addjob queue_0004 myjob 0
    }

    for {set i 0} {$i < 4} {incr i} {
        R 2 getjob from queue_0001
        R 2 getjob from queue_0002
        R 2 getjob from queue_0003
        R 2 getjob from queue_0004
    }

    set res [R 2 qscan busyloop importrate 1]
    set queues [lindex $res 1]
